<div class="{{ isset($addClass) ? $addClass : '' }} form-group has-feedback {{ $errors && $errors->has($name) ? 'has-error' : '' }}">
    @if (isset($label) && $label)
        <label class="control-label col-md-2 col-sm-2 col-xs-12 text-semibold">{{ $label }}</label>
    @endif
    <div class="col-md-{{ (isset($label) && $label) ? '10' : '12' }} col-sm-{{ (isset($label) && $label) ? '10' : '12' }} col-xs-12">
